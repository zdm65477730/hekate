...
{
	lv_obj_t *win = nyx_create_standard_window(SYMBOL_COPY" Nyx颜色主题");
	lv_win_add_btn(win, NULL, SYMBOL_HINT" 切换背景", _preset_bg_action);
	lv_win_add_btn(win, NULL, SYMBOL_SAVE" 保存与重载", _save_theme_color_action);
...
static lv_res_t _create_mbox_clock_edit(lv_obj_t *btn)
{
	...
	lv_mbox_set_text(mbox, "请为Nyx输入#C7EA46 日期#和#C7EA46 时间#，\n"
						   "此信息将应用于所有文件操作及菜单功能。\n"
						   "此操作不会修改实际的硬件时钟！");
...
disabled_or_cal0_issue:;
	...
	if (!error)
	{
		if (!nx_hoag)
		{
			...
			{
				strcat(txt_buf, "#FF8000 警告：##FFDD00 无手柄# 配对数据基于官方系统！");
				success = false;
			}

			if (!success)
				strcat(txt_buf,
					"\n\n#FFDD00 确保两个Joy-Con都已连接，#\n"
					"#FFDD00 并且在官方系统中将它们配对！#");

			if (cal_error)
				s_printf(txt_buf + strlen(txt_buf), "\n\n#FF8000 警告：获取IMU校准数据失败（%d）！#", cal_error);
		}
		else
		{
			s_printf(txt_buf,
				"提取到SD卡完成！\n"
				"保存到：#C7EA46 switchroot/switch.cal#\n\n");
			strcat(txt_buf, "#C7EA46 成功！#\n#C7EA46 找到Lite手柄数据#\n");
		}
	}
	else
	{
		if (!nx_hoag)
			...
			s_printf(txt_buf, "#FFDD00 无法获取Lite手柄信息！#\n#FFDD00 错误：%d#", error);
	}
...
lv_res_t create_win_nyx_options(lv_obj_t *parrent_btn)
{
	...
	lv_label_set_static_text(label_txt2, "为Nyx中所有#00FFC8 高亮#区域选择颜色。\n");
	...
	lv_label_set_static_text(label_txt2,
		"选择在Nyx启动时显示的屏幕。\n"
		"#FF8000 所有配置：##C7EA46 把更多设置合并到启动的空白处。#\n"
		"#FF8000 启动或更多设置：##C7EA46 使用经典的分割视图。#");
	lv_obj_set_style(label_txt2, &hint_small_style);
	...
	lv_label_set_recolor(label_txt2, true);
	lv_label_set_static_text(label_txt2,
		"允许您保存Switch和Joy-Con的MAC地址以及相关联的LTK信息。\n"
		"用于#C7EA46 安卓#和#C7EA46 Linux#。");
	lv_obj_set_style(label_txt2, &hint_small_style);
	...
	lv_label_set_static_text(label_txt, SYMBOL_MODULES_ALT" 数据校验");
	lv_obj_set_style(label_txt, th->label.prim);
	lv_obj_align(label_txt, line_sep, LV_ALIGN_OUT_BOTTOM_LEFT, LV_DPI / 4, LV_DPI / 4);
	...
	label_txt2 = lv_label_create(sw_h3, NULL);
	lv_label_set_static_text(label_txt2, "置为备份和还原完成的数据验证类型。\n"
		"可以取消而不会丢失系统备份/恢复。\n");
	lv_obj_set_style(label_txt2, &hint_small_style);
	lv_obj_align(label_txt2, label_txt, LV_ALIGN_OUT_BOTTOM_LEFT, 0, LV_DPI / 4);
	...
	lv_label_set_static_text(label_btn5, SYMBOL_CLOCK" 时间（偏移）");
...
void create_tab_options(lv_theme_t *th, lv_obj_t *parent)
{
	...
	lv_obj_t *label_txt2 = lv_label_create(sw_h2, NULL);
	lv_label_set_static_text(label_txt2, "选择注入时自动启动的启动项或者有效荷载。");
	lv_obj_set_style(label_txt2, &hint_small_style);
	...
	label_txt2 = lv_label_create(sw_h3, NULL);
	lv_label_set_static_text(label_txt2,
		"当从官方系统中执行关机操作后，设备会在 15 秒后唤醒。\n"
		"启用此功能可在下一次有效载荷注入时自动关闭电源。");
	lv_obj_set_style(label_txt2, &hint_small_style);
	...
	nyx_create_onoff_button(th, sw_h3, btn4, SYMBOL_REFRESH" 更新并重启至有效载荷", _update_r2p_action, true);
	lv_obj_align(btn4, line_sep, LV_ALIGN_OUT_BOTTOM_LEFT, 0, LV_DPI / 10);

	label_txt2 = lv_label_create(sw_h3, NULL);
	lv_label_set_recolor(label_txt2, true);
	lv_label_set_static_text(label_txt2,
		"若选定的启动项使用#FF8000 FSS0#引导，则会检测重启的有效载荷二进制文件，\n"
		"并强制将其更新为Hekate版本。\n\n\n\n");
	lv_obj_set_style(label_txt2, &hint_small_style);
