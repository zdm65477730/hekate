...
static int _dump_emmc_part(emmc_tool_gui_t *gui, char *sd_path, int active_part, sdmmc_storage_t *storage, emmc_part_t *part)
{
	...
	if (totalSectors > (sd_fs.free_clst * sd_fs.csize))
	{
		...
		if (!maxSplitParts)
		{
			strcpy(gui->txt_buf, "#FFDD00 没有足够的可用空间进行部分备份！#\n");
			lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
			manual_system_maintenance(true);
	...
	while (totalSectors > 0)
	{
		...
		while (res_read)
		{
			...
			{
				strcpy(gui->txt_buf, "#FFDD00 正在重试...#\n");
				lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
				manual_system_maintenance(true);
			}
...
multipart_not_allowed:
	...
	while (totalSectors > 0)
	{
		...
		if (numSplitParts != 0 && bytesWritten >= fileSize)
		{
			...
			if (res)
			{
				s_printf(gui->txt_buf, "\n#FF0000 打开文件 # #FFDD00 %s # #FFDD00 时出错（%d）！#\n", outFilename, res);
				lv_label_ins_text(gui->label_log, LV_LABEL_POS_LAST, gui->txt_buf);
				manual_system_maintenance(true);
...
void restore_emmc_selected(emmcPartType_t restoreType, emmc_tool_gui_t *gui)
{
	...
	if (!gui->raw_emummc)
		strcpy(txt_buf, "#FFDD00 这可能会使设备无法使用！#");
	else
		strcpy(txt_buf, "#FFDD00 这可能会使emuMMC无法使用！#");
	strcat(txt_buf, "\n\n#FFDD00 您确定要继续吗？#");
