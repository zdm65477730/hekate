...
lv_res_t create_window_partition_manager(bool emmc)
{
	...
	if (!emmc)
	{
		...
		win = nyx_create_standard_window(SYMBOL_CHIP" MMC分区管理器", NULL);
	...
	lv_label_set_static_text(lbl_and, "#FF8000 "SYMBOL_DOT" Android系统（用户）#");
	...
	if (!emmc)
	{
		lv_label_set_static_text(lbl_notes,
			...
			"注3：若找到适配的分区及安装文件，#C7EA46 刷写Linux#和#C7EA46 刷写Android#功能将自动刷写对应文件。\n");
	...
	case 1: // Android 11.
		...
		break;
	case 2: // Android 13+
		lv_label_set_static_text(label_btn, SYMBOL_DOWNLOAD"  刷写Android13+");
		break;
	}
