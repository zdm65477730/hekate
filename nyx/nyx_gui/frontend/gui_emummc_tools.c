...
static void _create_window_emummc()
{
	...
	lv_obj_t *win;
	if (!mbr_ctx.part_idx)
		win = nyx_create_standard_window(SYMBOL_DRIVE"  创建SD卡文件虚拟MMC", _action_emummc_window_close);
	else
		win = nyx_create_standard_window(SYMBOL_DRIVE"  创建SD卡分区虚拟MMC", _action_emummc_window_close);
...
{
	lv_obj_t *win = nyx_create_standard_window(SYMBOL_SETTINGS"  调整虚拟MMC", _action_win_change_emummc_close);
...
{
	lv_obj_t *win = nyx_create_standard_window(SYMBOL_EDIT"  虚拟MMC系统管理", NULL);
	...
	lv_label_set_recolor(label_txt4, true);
	lv_label_set_static_text(label_txt4,
		"将备份迁移至#C7EA46 SD卡文件虚拟系统#，或修复现有的#C7EA46 SD卡分区虚拟系统#。\n");
	lv_obj_set_style(label_txt4, &hint_small_style);
